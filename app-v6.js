...
function getSavedSheets() {
  try {
    ...
    if (Array.isArray(saved) && saved.length) return saved.filter((sheet) => sheet && typeof sheet === "object");
    ...
    const validLegacy = legacy.filter((sheet) => sheet && typeof sheet === "object");
    if (validLegacy.length) setSavedSheets(validLegacy);
    return validLegacy;
  } catch {
    return [];
  }
...
function loadSheet(id, message = "Opened") {
  ...
  if (!sheet || !sheet.state || !sheet.inputs) {
    showToast("This saved costing is incomplete and could not be opened.");
    return;
  }
  currentEditingId = sheet.id;
...
function renderDashboard() {
  ...
  const sheets = getSavedSheets();
  const visibleSheets = sheets.filter((sheet) => String(sheet.styleName || "Untitled").toLowerCase().includes(query));
  els.savedSheetCount.textContent = sheets.length;
  els.uniqueStyleCount.textContent = new Set(sheets.map((sheet) => String(sheet.styleName || "Untitled").toLowerCase())).size;
  ...
  visibleSheets.forEach((sheet) => {
    ...
    row.innerHTML = `
      <div class="saved-sheet-identity"><p>${escapeHtml(sheet.styleName || "Untitled")}</p><small>Updated ${sheet.updatedAt ? new Date(sheet.updatedAt).toLocaleDateString() : "Unknown date"}</small></div>
    ...
    row.querySelector(".open-sheet").disabled = !sheet.state || !sheet.inputs;
    row.querySelector(".edit-sheet").disabled = !sheet.state || !sheet.inputs;
    row.querySelector(".print-sheet").disabled = !sheet.state || !sheet.inputs;
